...
# =============================================================================
# GUARDAR COMO: resma4.9_fixed.py
# COMPATIBLE CON: resma_checkpoint_v4_6.pkl
# FIX CRÍTICO: Conversión automática de listas a numpy arrays al cargar checkpoint
...
class NeuralNetworkRESMA:
    ...
    def _generate_realistic_modular_network(self) -> nx.Graph:
        ...
        if density_current < target:
            logging.info(f"  Fase 3: Densificación modular...")
            ...
            logging.info(f"     ✓ Densidad final: {density_final:.2%} (+{added} aristas)")
...
def guardar_checkpoint(data: Dict[str, Any], filename: str = CHECKPOINT_FILE):
    ...
    try:
        ...
        if size_mb < 0.1:
            logging.warning("⚠️  Checkpoint muy pequeño")
